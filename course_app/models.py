...
def upload_image_path(instance, filename):
    ...
    final_name = (
        f"{instance.get_jalali_date_for_url()}/{instance.title}-{random_num}{ext}"
    )
...
def upload_video_path(instance, filename):
    ...
    name, ext = get_filename_ext(filename)
    final_name = (
        f"{instance.course.id}-{instance.position}-{instance.title}-{random_num}{ext}"
    )
...
class CourseManager(models.Manager):
    ...
    def get_course_by_category(self, category_name):
        return (
            self.get_queryset()
            .filter(categories__title__iexact=category_name, status=True)
            .distinct()
        )
    ...
    def get_popular_course(self):
        return (
            self.get_queryset()
            .annotate(q_count=models.Count("student"))
            .filter(status=True)
            .order_by("-q_count", "-publish_time")
            .distinct()
        )
...
class CourseCategory(models.Model):
    title = models.CharField(
        max_length=200, unique=True, verbose_name="عنوان دسته بندی"
    )
...
class Course(models.Model):
    ...
    tags = models.ManyToManyField(
        CourseTag, blank=True, related_name="course_tag", verbose_name="برچسب"
    )
    level = models.CharField(
        max_length=2, choices=LEVEL_CHOICES, default="b", verbose_name="سطح دوره"
    )
    language = models.CharField(
        max_length=2, default="fa", choices=LANGUAGE_CHOICES, verbose_name="زبان دوره"
    )
    ...
    def get_absolute_url(self):
        return reverse("course:course_detail", args=[self.pk, self.slug])
    ...
    def category_to_str(self):
        return " - ".join(
            [category.title for category in self.categories.get_active_category()]
        )
