...
from .mixins import (
    ...
    CourseValidMixin,
    CourseFieldMixin,
    BlogCreateFieldMixin,
    BlogCreateValidMixin,
    TeacherBlogUpadteMixin,
    TeacherCourseUpadteMixin,
    VideoUpdateMixin,
)
    ...
    def get_success_url(self):
        ...
        try:
            if self.request.GET.get("next"):
                return resolve_url(
                    f"http://127.0.0.1:8000{self.request.GET.get('next')}"
                )
                # todo : set site url
        ...
        if form.is_valid():
            ...
            try:
                coupon = CouponCode.objects.get(
                    code__exact=code, start__lte=time, end__gte=time, status=True
                )
...
def checkout(request):
    ...
    if cart.coupon_code:
        ...
        try:
            coupon = CouponCode.objects.get(
                code__exact=cart.coupon_code.code,
                start__lte=time,
                end__gte=time,
                status=True,
            )
...
class MyCourses(LoginRequiredMixin, ListView):
    def get_queryset(self):
        user = self.request.user
        courses = user.student_courses.get_publish_course().order_by(
            "-course_app_course_student.id"
        )
        return courses
...
# add course view
class CourseAdd(
    LoginRequiredMixin, CourseValidMixin, CourseFieldMixin, TeacherMixin, CreateView
):
    model = Course
...
class PaymentDetail(LoginRequiredMixin, DetailView):
    def get_object(self):
        ...
        order = get_object_or_404(
            Order.objects.filter(user_id=self.request.user.id), pk=pk
        )
        return order
